...
show_data = False   #用于显示
test_mode = False       #True为测试模型，False为训练模型
...
def get_distance(p1,p2):                #获取两点间距离
...
class lidar(object):
    def __init__(self,p,distance=200,angle=0):
        self.x = p[0]
        self.y = p[1]
        self.distance = distance   #雷达探测距离  1个像素代表1cm
        self.angle = 0
        self.lidar =(0,0) #(角度，距离)
        self.data = []    #存储雷达探测到的坐标点
        self.data_2 = []  #存储雷达坐标点相对于雷达中心的角度和距离
    def scan(self):
        dis = []
        for i in range(0,360,1):    #0-359
            for j in range(1,40):
            ...
            if(self.x>0 and self.x<XDIM and self.y>0 and self.y<YDIM):      
                ...
                if(len(self.data_2)<360):
                    self.data_2.append([i,scan_dis])            #将扫描到的角度和距离传入数组中
                else:
...
class NavigationEnv(object):
    def __init__(self,level=1,actions=8,scope=15):
        ...
        self.scope = scope     #人为设置到达目标的最大范围
        self.node = []
        self.old_goalpoint = []
    ...
    def random_point(self,length=20,width=20):  #随机生成点，用于car初始点的生成
        flag = False
        while (not flag):        
    ...
    def close_to_goalpoint(self,dis_x,dis_y,length=20,width=20):   #随机点距离目标点的距离
        flag = False
    ...
    def goal_point(self,length=5,width=5):            #随机生成目标点
        flag = False
        while (not flag):        
            x = random.randint(int(length//2+1),int(XDIM-length//2-1))
            y = random.randint(int(width//2+1),int(YDIM-width//2-1))
    ...
    def check_goal(self,p,size=[20,20]):      #检查当前小车的状态，返回奖励值
...
Env = NavigationEnv()                     #环境初始化
px,py,pl,pw = Env.random_point()
car_A = car(px,py,pl,pw)           #随机初始化移动体和雷达位置以及相应参数
...
def observe():
    ...
    if len(state) < 400:
        for i in range(20):
            state.append([Env.goal[0]-car_A.positon()[0],Env.goal[1]-car_A.positon()[1]])       #car_A.positon()是一个tuple，需要拆分
        for i in range(20):
...
class Qnetwork(object):
    def __init__(self,size):
        ...
        self.layerAC,self.layerVC=tf.split(1,2,self.layer2)           #(self.layer2,2,1)版本不同
...
def processState(states):                    #将多维数据转换为一维数据
...
num_episodes=20000       #迭代代数
pre_train_steps=5000
max_epLength=150         #每代最大次数

path="./dqn"
h_size=512
tau=0.001                #targetQ跟随主Q的学习率
...
myBuffer=experience_buffer()     #经验回放
e=startE
...
with tf.Session(config=config) as sess:
    ...
    for i in range(num_episodes+1):
        ...
        #px,py,pl,pw = Env.random_point()    #产生随机点
        if(test_mode == False):
        ...
        if test_mode ==True:
            ...
            Env.goal_point()  # 随机目标点
            Env.reset(map_level)  # 初始化环境
        ...
        while j < max_epLength:
            ...
            lidar_A.scan()       #在新的位置雷达开始扫描障碍物     
            s1,r,d = observe()    #获得环境状态值
            s1 = processState(s1)  #将状态值转换为1维数值
            total_steps+=1
            if(show_data == True):
                print('r',r)
            ...
            if total_steps > pre_train_steps:
                ...
                if total_steps %(updata_freq)==0:
                    ...
                    if(i%10==0 and j%100 ==0):
                        plt_targetQ.append(targetQ[10])   #数据用于绘制曲线
